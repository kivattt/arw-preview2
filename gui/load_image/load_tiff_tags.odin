...
get_jpeg_image_preview_from_file_tiff_tags :: proc(
	fileHandle: os.Handle,
	previewImageStartTag: u16,
	previewImageLengthTag: u16,
) -> (
	previewImage: []u8,
	...
	for i: u16 = 0; i < numDirEntries; i += 1 {
		...
		if valueOffsetIsValue {
			if tag == previewImageStartTag {
				previewImageStart = valueOffset
			} else if tag == previewImageLengthTag {
